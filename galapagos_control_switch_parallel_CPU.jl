    ...
    struct RunParameters
        ...
        EUC_model::Int #Look at the function, but 0 = constant forcing, 1 = fourier-based is the data based waveforms
    ...
    function run_simulation(p::RunParameters)
        ...
        if EUC_model == 0
            @info "Using constant forcing for EUC"
        ...
        elseif EUC_model == 1
            @info "Using Fourier-based forcing for EUC (NOT INPUTED IN YET)"
        ...
        if model_type == "hydrostatic" # Placeholder for model type selection
        ...
        elseif model_type == "nonhydrostatic" # Placeholder for another model type
        ...
        simulation = Simulation(model, Δt=Δt₀,
                                stop_time = 365days, # when to stop the simulation
        )
        ...
        metadata = DataFrame(
            ...
            Stratification         = params.N²₀,

            InitialDeltaT          = string(Δt₀),
            StopTime               = string(simulation.stop_time),
    ...
    function build_master_metadata(rundir)
        ...
        XLSX.writetable(masterfile,
                        collect(eachcol(master)),
                        names(master))

        println("Master metadata written to:")
        println(masterfile)
