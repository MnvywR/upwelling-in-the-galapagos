...
elseif bathymetry_mode == 2
    ...
    ds = NCDataset(joinpath(rundir,"galap.nc"))
    ...
    depth[(-30 .< depth) .& (depth .< 0)] .= 0 #clipping between 20-0 depth for gradiant spikes
...
params = (; Lx = Lx_real,
        ...
        σ = 20days, # s (relaxation timescale for sponge layer) 
...
initial_stop_time = 300days   # Phase 1: spin-up
final_stop_time   = 500days   # Phase 2: total simulation end
...
#+++ Create a timestamped run folder for this simulation's outputs
...
bathy_tag = bathymetry_mode == 0 ? "no_bathymetry" :
            bathymetry_mode == 1 ? "gaussian_bathymetry" :
                                   "real_bathymetry"

wind_tag = wind == 0 ? "no_wind" : "wind"

beta_tag = beta_switch == 0 ? "no_beta" : "beta"

run_timestamp = Dates.format(now(), "yyyymmdd_HHMMSS_sss")

run_tag = "$(bathy_tag)_$(beta_tag)_$(wind_tag)_$(run_timestamp)"
...
#+++ Metadata: record the control-board switches and key physical/grid parameters
# used for this run, timestamp it, and write it into the run's own folder as an xlsx.
metadata = DataFrame(
    RunTag             = run_tag,
    Timestamp          = run_timestamp,
    OutputFolder       = output_dir,
    Architecture       = string(arch),
    BathymetryMode     = bathymetry_mode,
    Bathymetry         = bathy_tag,
    SmoothedBathymetry = Smoothing_bathymetry,
    Wind               = wind,
    WindTag            = windtag,
    BetaPlane          = beta_switch,
    BetaTag            = beta_tag,
    HeatSaltFlux       = H_S_flux,
    Nx                 = params.Nx,
    Ny                 = params.Ny,
    Nz                 = params.Nz,
    Lx                 = params.Lx,
    Ly                 = params.Ly,
    Lz                 = params.Lz,
    Stratification_N2  = params.N²₀,
    SpongeTimescale_s  = string(params.σ),
    U_b                = params.u_b,
    V_b                = params.v_b,
    EUC_Umax           = Umaxᵥ,
    EUC_z0             = zₒᵥ,
    ...
    EUC_sigma_z        = σ_zᵥ,
    EUC_sigma_y        = σ_yᵥ,
    InitialDeltaT_s    = string(Δt₀),
    InitialStopTime = string(initial_stop_time),
    FinalStopTime   = string(final_stop_time),
    InterpolatedIC     = interpolated_IC,
    MassFlux           = mass_flux,
    LES                = LES,
    ExternalForcing    = ext_forcing,
    OutputPrefix       = saved_output_prefix,
)
...
# ============================================================
# PHASE 1 OUTPUTS
# Days 0 -> final stop time
# ============================================================

    @info "Phase 1: saving top_xy and midy outputs through day $initial_stop_time"
simulation.output_writers[:surface_slice_writer] =
    NetCDFWriter(model, (; u, v, w, T, S, vorticity_z, KE_u, KE_v, KE_w, KE_total);
    filename = joinpath( joinpath(output_dir, "top_xy.nc")),
...
@info "Phase 2: starting upwelling output at day $initial_stop_time"

# Change the stopping time from 300 days to 500 days
simulation.stop_time = final_stop_time
...
simulation.output_writers[:IC_writer] =
    NetCDFWriter(model, (; u, v, w, T, S, vorticity_z, KE_u, KE_v, KE_w, KE_total);
                ...
                schedule = TimeInterval(final_stop_time),  # only saves at the end
                overwrite_existing = overwrite_existing)
