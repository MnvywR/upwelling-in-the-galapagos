    ...
    elseif wind==1
        ...
        ds = NCDataset("wind_reg.nc") #load wind data 
    ...
    simulation.output_writers[:surface_slice_writer] =
        NetCDFWriter(model, (; u, v, w, T, S, vorticity_z, KE_u, KE_v, KE_w, KE_total); filename = joinpath(rundir, "top_real_GPU.nc"),
    ...
    simulation.output_writers[:y_slice_writer] =
        NetCDFWriter(model, (; u, v, w, T, S, vorticity_z, KE_u, KE_v, KE_w, KE_total); filename= joinpath(rundir, "midy_real_GPU.nc"),
                        schedule=TimeInterval(8640seconds), indices=(:, Int(params.Ny/2), :), 
                        overwrite_existing = overwrite_existing)    

    simulation.output_writers[:xy_75_depth_writer] =
        NetCDFWriter(model, (; u, v, w, T, S, vorticity_z, KE_u, KE_v, KE_w, KE_total); filename = joinpath(rundir, "upwelling_75m_GPU.nc"),
