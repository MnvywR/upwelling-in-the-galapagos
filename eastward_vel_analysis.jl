...
#----------------------------------------------
# MERIDIONAL STRUCTURE ANALYSIS
#----------------------------------------------
#=
Takes a time series at a single longitude and depth, but varies across all latitudes
For each latitude point, it extracts that meridional slice 
Performs FFT (Fast Fourier Transform) on that time series ( takes the amplitude and phase of that signal)
Finds the amplitude and phase at a target period (e.g., 18 days for TIW)
Returns amplitude and phase as arrays varying by latitude
=#
...
bands = [
    ("Yanai / Inertia-Gravity (3-15d)", 8.0),
    ("TIW (17-33d)", 18.0),
    ("TIW Broad (17-33d)", 33.0),
    ("Intraseasonal Kelvin / MJO (30-90d)", 60.0),
    ("Annual Cycle (~365d)", 365.0),
    ("ENSO (2-7 yr)", 1460.0),
]
...
for (label, T) in bands
    ...
    Plots.vline!(plot_amp, [0.0], label="Equator", linestyle=:dash, color=:black, lw=2)
    display(plot_amp)
    ...
    Plots.vline!(plot_phase, [0.0], label="Equator", linestyle=:dash, color=:black, lw=2)
    display(plot_phase)
end
